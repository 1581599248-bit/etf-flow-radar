...
- 统计范围对账：同时保存“全部场内ETF”“股票ETF（含跨境）”“A股股票ETF”三个口径，避免拿不同范围的媒体数字直接比较。
- 二级市场资金：若存在与交易日严格同日的“主力净流入”数据，单独展示为二级市场订单流；绝不与ETF净申购/赎回混为一个字段。
...
它用于与采用成交均价的 Wind/资讯统计做交叉核验，但不会覆盖主字段。

### 2. 二级市场主力资金：单独命名空间

仅当行情源的 `数据日期` 与目标交易日完全相等时，保存：

`flowMetrics.secondaryMarketOrderFlow`

数据来自ETF二级市场“主力净流入-净额”等订单流字段。若周末/节假日行情源把 `数据日期` 滚到自然日，系统直接标记 `unavailable`，**绝不把周日快照回填到周五**。
...
因此看到外部“ETF净流出200亿元”时，必须先确认它说的是全部ETF、股票ETF（含跨境）、A股股票ETF，还是二级市场主力订单流，再比较数字。
...
4. 生成 schema v6：一级市场、二级市场、三个市场范围、ETF级审计字段与每日单ETF缓存。
5. 构建静态页面并运行前端测试。
