...
def build_snapshot(day: date, current: pd.DataFrame | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "methodology": {
            ...
            "classification": f"SW2021_L1_ETF_V2：行业名称与代码采用申万行业分类标准2021版31个一级行业；ETF按交易所简称与基金全称映射到主要暴露行业，每只ETF只进入一个主要组。当前有{len(industry_parent_ids)}个一级行业实际存在可分析ETF；对ETF集中度高、市场关注度高的热门一级行业进一步拆分子组（如电子拆分为半导体、芯片、消费电子，计算机拆分为AI算力、软件信创等），子组归属相应一级行业，不重复计数；不再设置跨行业主题组，未能明确对应申万一级行业、宽基或风格策略的产品不进入资金分析。",
